...
2. 勾選一個或多個題庫並建立場次；勾選多份時會先合併題目，再依每場題數隨機抽題。
3. 用無痕視窗開學生連結，或開 `/join` 後輸入加入碼，模擬 2-3 位學生加入。
4. 手機連同一個 Wi-Fi，開終端機顯示的手機測試網址。
5. 老師按開始遊戲，學生答題。
6. 題目會自動倒數，時間到自動公布答案；老師查看統計後按下一題。
7. 結束後可在老師頁的「歷史場次」查看排行榜、每題答題狀況，或建立錯題重練。

學生在遊戲結束後可以查看自己的錯題與未作答題目。老師可在每題公布答案後查看全班答對、答錯與未作答名單，也可以從歷史場次上方的留言區傳送訊息給本場的個別學生；學生看到留言後，留言會在 5 秒後自動消失。學生尚未看到的留言會保留到下次開啟頁面。
...
儲存後重新整理老師頁，新的測驗就會出現在題庫勾選清單。檔案名稱可以自訂，但 JSON 裡的 `id` 不要和其他測驗重複。
...
歷史場次只保留最近 5 天。後端啟動時、老師讀取歷史紀錄時，以及服務持續運作期間，都會自動刪除超過 5 天的場次；相關學生與作答資料會透過資料庫關聯一起刪除。
